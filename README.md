...
`.\xextool.exe -o default.xex -c u -e u ` - (uncompress& unencrypt)
`.\xextool.exe -i default.xex > dump.txt` - get file info, also about compressing & encryption

(if xextool output error, then it means that .xex broken or modified and you should try another file)
...
_(important register 14 , you can use modified XenonAnalyse from this fork: (https://github.com/testdriveupgrade/XenonRecompUnlim) with auto-get of r14(will be outputed in console after beginning) of r14, but eventhough check whem manual by this instruction (if one of it is the same, when auto-get got them correctly ):
...
get UnleashedRecomp,  and insert your PPC to src (comment hard-coded functions): https://github.com/hedge-dev/XenonRecomp/issues/125#issuecomment-2746404991
